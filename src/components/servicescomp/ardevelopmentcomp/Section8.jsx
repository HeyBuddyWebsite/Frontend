...
const Pagenation = () => {
  ...
  const techStack = {
    task1: ["Unity AR Foundation", "ARCore (Android)", "ARKit (iOS)", "Vuforia", "Wikitude"],
    task2: ["8th Wall (WebAR)", "Niantic Lightship", "Spark AR (Meta)", "Lens Studio (Snap)", "ZapWorks"],
    task3: ["Three.js", "A-Frame", "React XR", "Blender", "Autodesk Maya"],
    task4: ["Azure Spatial Anchors", "Google Cloud Anchors", "Echo3D", "PlayCanvas", "Babylon.js"]
  };
    ...
    >
      <div className="py-6 mb-8 text-center">
        ...
        <p className="text-lg lg:text-xl text-gray-400 max-w-4xl mx-auto leading-relaxed">
          Our Augmented Reality toolkit spans from native mobile frameworks to sophisticated web-based AR solutions, ensuring reaching users wherever they are.
        </p>
      </div>

      <nav className="flex flex-nowrap overflow-x-auto scrollbar-hide border-b border-white/5 mb-8 justify-center">
        {[
          { id: "task1", label: "Mobile AR" },
          { id: "task2", label: "Web & Social AR" },
          { id: "task3", label: "WebXR & Modeling" },
          { id: "task4", label: "Cloud & Rendering" }
        ].map(tab => (
          <TabSelector
            key={tab.id}
