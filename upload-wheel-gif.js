...
const IMAGE_PATH = path.join(__dirname, 'public/Images/wheel-unscreen.gif');
...
async function main() {
  ...
  try {
    ...
    // Save result to file
    fs.writeFileSync('upload-wheel-gif-result.json', JSON.stringify({ [path.basename(IMAGE_PATH)]: url }, null, 2));
    console.log('\n✅ Result saved to upload-wheel-gif-result.json');
  } catch (error) {
